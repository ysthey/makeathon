...
//        } else {
         ...
         if (mTranslator.validateStatusMessage(returnMessage, Robot.COMMAND_GET_INPUT_VALUES)) {
            scaledValue = mTranslator.getSensorReadingFromMessage(returnMessage, Robot.SCALED_VALUE);
    ...
    public void startListening() {
        if (mSensor.getPort()==-1) {
            mCommander.onSensorReport(mSensor, Commander.STATUS_NO_SENSOR, 0);
        } else {
        isListening = true;
        ...
        while (isListening && btComms.isConnected() && !Thread.currentThread().isInterrupted()
            ...
            mCommander.onSensorReport(mSensor, Commander.STATUS_OK, scaledValue);
            if (scaledValue <= (mUpperLimit) && scaledValue >= (mLowerLimit) || ((mUpperLimit2 - mLowerLimit2 != 0) &&
                ...
                mCommander.onSensorReport(mSensor, Commander.STATUS_COMPLETE, scaledValue);
                targetFound = true;
                break;
            }
        }
