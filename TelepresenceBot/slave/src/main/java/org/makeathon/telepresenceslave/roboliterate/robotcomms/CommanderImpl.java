...
import org.makeathon.telepresenceslave.roboliterate.instructions.AndroidStopSound;
import org.makeathon.telepresenceslave.roboliterate.instructions.Instruction;
import org.makeathon.telepresenceslave.roboliterate.instructions.Program;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.Robot.Motor;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.Robot.Sensor;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.ev3.ByteTranslatorEV3;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.ev3.MotorListenerEV3;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.nxt.ByteTranslatorNXT;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.nxt.DistanceSensorListenerNXT;
import org.makeathon.telepresenceslave.roboliterate.robotcomms.nxt.MotorListenerNXT;
...
if (returnMessage!=null) {
        if (returnMessage[0]==0 && returnMessage[2]==2) {
            ...
            mListener.onRobotTypeDetected(Robot.MODEL_EV3);
            ...
            mListener.onRobotTypeDetected(Robot.MODEL_NXT);
            }
}
    ...
    public void doRobotMove(int throttle, int distance, boolean waitUntilComplete) {
        ...
        else if (Robot.Motor.LEFT.getPort()!=-1 && Robot.Motor.LEFT.getPort()!=-1) {
            byte[] message_leftMotor = mTranslator.getMotorMessage(Robot.Motor.LEFT, throttle, distance, true);
            byte[] message_rightMotor = mTranslator.getMotorMessage(Robot.Motor.RIGHT, throttle, distance, true);
    ...
    public void doRobotMove(int portIndex, int throttle, int degrees, boolean blockInstruction) {
        Robot.Motor motor = Robot.Motor.getMotorForPortIndex(portIndex);
    ...
    public void doRobotRotate(int throttle, int degrees, boolean waitUntilComplete) {
        ...
        if (Robot.Motor.LEFT.getPort()!=-1 && Robot.Motor.LEFT.getPort()!=-1) {
            byte[] message_leftMotor = mTranslator.getMotorMessage(Robot.Motor.LEFT, throttle, degrees, false);
            byte[] message_rightMotor = mTranslator.getMotorMessage(Robot.Motor.RIGHT, -throttle, degrees, false);
            sendMessageToRobot(message_leftMotor);
            sendMessageToRobot(message_rightMotor);
            if (waitUntilComplete) {
                waitForRobot(LISTENER_MOTORS_MOVE, 0, 0, 0, 0, 0);
    ...
    public void doRobotTurnArm(int throttle, int degrees, boolean waitUntilComplete) {
        ...
        if (Robot.Motor.ARM.getPort()!=-1) {
            byte[] message_armMotor = mTranslator.getMotorMessage(Robot.Motor.ARM, throttle, degrees, false);
    ...
    public void waitForRobot(int listenerType, int duration, int lowerTarget, int upperTarget, int lowerTarget2, int upperTarget2) {
        ...
        switch (listenerType) {
            case LISTENER_MOTORS_ARM:
                    mRobotListener = (Robot.getRobotType()==Robot.MODEL_EV3) ?
